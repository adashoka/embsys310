...
swapPointersAsm
    LDR R2,[R0] //load value stored in Mem location pointed to by R0 (say x)
    LDR R3,[R1] //load value stored in Mem location pointed to by R1 (say y)
    STR R3,[R0] //store y into mem location pointed to by R0 (earlier x)
    STR R2,[R1] //store x into mem location pointe to by R1 (earlier y)
    BX LR
    END
